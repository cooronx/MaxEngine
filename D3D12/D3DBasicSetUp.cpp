...
void D3DBasicSetUp::OnResize() {
  ...
  ThrowIfFailed(swap_chain_->ResizeBuffers(
      swap_chain_buffer_cnt_, client_width_, client_height_,
      back_buffer_format_, DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH));
  ...
  for (UINT i = 0; i < swap_chain_buffer_cnt_; i++) {
    ThrowIfFailed(
        swap_chain_->GetBuffer(i, IID_PPV_ARGS(&swap_chain_buffer_[i])));
    d3d_device_->CreateRenderTargetView(swap_chain_buffer_[i].Get(), nullptr,
                                        rtvHeapHandle);
  ...
  depthStencilDesc.SampleDesc.Quality =
      MSAA4x_state_ ? (MSAA4x_quality_level_ - 1) : 0;
  ...
  ThrowIfFailed(d3d_device_->CreateCommittedResource(
      &heap_property, D3D12_HEAP_FLAG_NONE, &depthStencilDesc,
  ...
  d3d_device_->CreateDepthStencilView(depth_stencil_buffer_.Get(), &dsvDesc,
                                      DepthStencilDesc());
  ...
  scissor_rect_ = {0, 0, static_cast<LONG>(client_height_),
