...
template <class DataType> class UpLoadBuffer
{
        ...
        : is_constant_(is_constant)
    {
        ...
        if (is_constant_)
        {
            element_byte_size_ = HelperFuncs::CalculateConstantBufferByteSize(element_byte_size_);
        }
        /// 创建一个上传堆
        auto property = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
        auto resource = CD3DX12_RESOURCE_DESC::Buffer(element_count * element_byte_size_);
        ThrowIfFailed(device->CreateCommittedResource(&property, D3D12_HEAP_FLAG_NONE, &resource,
